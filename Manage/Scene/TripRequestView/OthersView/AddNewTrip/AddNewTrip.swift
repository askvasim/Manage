...
struct AddNewTrip<VM: AddNewTripViewModel>: View {
    @StateObject var viewModel: VM
    @InjectedObject private var navigator: DefaultAddNewTripNavigator
    ...
    var body: some View {
        ...
        }, title: "Create Trip", greetings: "", userName: "", userProfile: "", homeScreen: false, otherScreens: false, profileScreen: false, withBackButton: true, backButton: false, stopButton: false, startButton: false, filterButton: false, editButton: false, deleteButton: true, shouldPerformBack: true) {
            // Action
        } stopTapped: {
            // Action
        } startTapped: {
    ...
    private func editFields(tripData: TripList) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            Button {
                ...
                ZStack {
                    Image(uiImage: image ?? profilePicture)
                        .resizable()
            ...
            Button(action: saveTrip) {
                Text("REQUEST TRIP")
                    ...
                    .frame(maxWidth: .infinity, maxHeight: 44)
                    .padding()
                    .background(AppColor.appGrayFont)
                    .foregroundColor(AppColor.appPrimary)
                    .cornerRadius(10)
            }
//            .padding(.top, 20)
